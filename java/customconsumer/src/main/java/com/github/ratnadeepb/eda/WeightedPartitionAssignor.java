...
public class WeightedPartitionAssignor extends AbstractPartitionAssignor {
    ...
    public Map<String, List<TopicPartition>> assign(Map<String, Integer> partitionsPerTopic,
        ...
        subscriptions.forEach((memberId, subscription) -> memberAssignmentInfo.put(memberId,
                getMemberAssignmentInfo(subscription.userData())));
        ...
        subscriptions.forEach((key, value) -> memberAssignmentInfo.get(key).getAssignments().forEach(topicPartition -> {
        ...
        topicMemberPortions
                .forEach((topic, topicMemberPortion) -> topicMemberPortion.forEach((memberId, remainingToFill) -> {
    ...
    private static List<TopicPartition> allPartitionsSorted(Map<String, Integer> partitionsPerTopic,
        ...
        topics.forEach(topic -> allPartitions
                .addAll(AbstractPartitionAssignor.partitions(topic, partitionsPerTopic.get(topic))));
    ...
    private static Map<String, List<Integer>> groupPartitionsByTopic(List<TopicPartition> partitions) {
        ...
        partitions.forEach(partition -> topicPartitions.computeIfAbsent(partition.topic(), k -> new ArrayList<>())
