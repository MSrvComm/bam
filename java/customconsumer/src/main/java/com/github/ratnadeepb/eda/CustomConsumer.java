...
public class CustomConsumer {
    final Logger mLogger = LoggerFactory.getLogger(CustomConsumer.class.getName());
    ...
    private class ConsumerRunnable implements Runnable {
        ConsumerRunnable(CountDownLatch latch) {
            ...
            mConsumer.subscribe(Collections.singletonList("OrderTopic"), new ConsumerRebalanceListener() {
                ...
                public void onPartitionsRevoked(Collection<TopicPartition> collection) {
                    mLogger.info("Revoking partitions");
                }

                @Override
                public void onPartitionsAssigned(Collection<TopicPartition> collection) {
                    mLogger.info("Assigning partitions");
                }
            });
        }
        ...
        public void run() {
            ...
            WeightedPartitionAssignor.setWeight(1);
            try {
                while (true) {
